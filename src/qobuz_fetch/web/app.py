...
from qobuz_fetch.web.csrf import (
    CSRFMiddleware,
...
async def _lifespan(_app: FastAPI):
    ...
    if _os.environ.get("QF_CHECK_VOLUMES") == "1":
...
async def dashboard(request: Request):
    ...
    from qobuz_fetch.ui_cli.prompts import _read_fetch_log
...
async def do_search(request: Request, q: str = Form("", max_length=500)):
    ...
    if query:
        try:
            ...
            from qobuz_fetch.library.catalog import album_quality_label, album_year
...
def _get_token():
    from qobuz_fetch.api.auth import load_qobuz_token
...
def _write_creds(user_id, auth_token) -> bool:
    ...
    Delegates to qobuz_fetch.api.auth.write_streamrip_creds so the web
    Settings path and the env-var sync share one credential writer."""
    from qobuz_fetch.api.auth import write_streamrip_creds
    return write_streamrip_creds(user_id, auth_token)


def start():
    import uvicorn
    uvicorn.run("qobuz_fetch.web.app:app", host=cfg.WEB_HOST, port=cfg.WEB_PORT, workers=1)
